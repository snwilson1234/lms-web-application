...
class StudentAccountManager(BaseUserManager):
    ...
    def create_user(self, email, username, firstname, lastname, major, year, password=None):
        if not email:
    ...
    def create_superuser(self, email, username, firstname, lastname, major, year, password):
...
class StudentAccount(AbstractBaseUser):
    ...
    REQUIRED_FIELDS = ['username','firstname','lastname','major','year']
...
# class IntructorAccount(AbstractBaseUser):
    # email           = models.EmailField(verbose_name="email",max_length=60, unique=True)
